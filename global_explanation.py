...
class GlobalExplanation:
    def __init__(self, xplain_obj):
        ...
        self.global_expl_info = {"map_rules": {}, "map_rules_abs": {}, "map_av": {}, "map_avM": {},
                                 "map_a": {k1: {"d": 0, "cnt": 0} for k1 in self.attr_list},
                                 "map_aM": {k1: {"d": 0, "cnt": 0} for k1 in self.attr_list}}
        self.map_predicted_class = {
            k: {"map_rules": {}, "map_rules_abs": {}, "map_av": {}, "map_avM": {},
                "map_a": {k1: {"d": 0, "cnt": 0} for k1 in self.attr_list},
                "map_aM": {k1: {"d": 0, "cnt": 0} for k1 in self.attr_list}} for k in
            self.XPLAIN_obj.classes}
    ...
    def getGlobalExplanation(self):
        ...
        for instance in self.XPLAIN_obj.explain_dataset:
            ...
            self.map_predicted_class[target_class]["map_a"] = {attr_list[i]: updateD_Cnt(
                self.map_predicted_class[target_class]["map_a"][attr_list[i]], diff_s[i]) for i in
                range(0, len(attr_list))}
            self.map_predicted_class[target_class]["map_aM"] = {attr_list[i]: updateD_Cnt(
                self.map_predicted_class[target_class]["map_aM"][attr_list[i]], diff_s[i],
                absV=True) for i in range(0, len(attr_list))}

            attr_value_list = getAttrValueList(e.instance)
            self.map_predicted_class[target_class]["map_rules"], \
            self.map_predicted_class[target_class]["map_rules_abs"] = self.updateRuleMapping(
                self.map_predicted_class[target_class]["map_rules"],
                self.map_predicted_class[target_class]["map_rules_abs"], e.map_difference,
                e.diff_single, e.instance)

            for i in range(0, len(attr_value_list)):
                k = attr_value_list[i]
                self.map_predicted_class[target_class]["map_av"] = self.initD_Cnt(k,
                                                                                  self.map_predicted_class[
                                                                                      target_class][
                                                                                      "map_av"])
                self.map_predicted_class[target_class]["map_avM"] = self.initD_Cnt(k,
                                                                                   self.map_predicted_class[
                                                                                       target_class][
                                                                                       "map_avM"])
                self.map_predicted_class[target_class]["map_av"][k] = updateD_Cnt(
                    self.map_predicted_class[target_class]["map_av"][k], e.diff_single[i],
                    absV=False)
                self.map_predicted_class[target_class]["map_avM"][k] = updateD_Cnt(
                    self.map_predicted_class[target_class]["map_avM"][k], e.diff_single[i],
                    absV=True)
            cnt = cnt + 1
            # print(".",end="")
            print(cnt)
    ...
    def computeGlobalInfo(self):
        for key in self.global_expl_info.keys():
            ...
            self.global_expl_info[key] = {k: {'d': sum(d[k]['d'] for d in dict1 if k in d),
                                              'cnt': sum(d[k]['cnt'] for d in dict1 if k in d)} for
                                          k in all_keys}
    ...
    def computeAvgD_Cnt(self, mapS):
        map_avg = sortByValue(
            {k: (mapS[k]["d"] / mapS[k]["cnt"] if mapS[k]["cnt"] != 0 else 0) for k in mapS.keys()})
        return map_avg
    ...
    def getRM(self, r, instT, sep=", "):
        if type(r) == str:
            ...
            for k in r.split(sep):
                rule1 = rule1 + instT[int(k) - 1].variable.name + "=" + instT[
                    int(k) - 1].value + ", "
            rule1 = rule1[:-2]
        return rule1
    ...
    def updateRuleMapping(self, map_rules, map_rules_abs, e_map_difference, single_diff,
                          e_instance):
        for k in e_map_difference:
            ...
            map_rules[r] = {"d": map_rules[r]["d"] + e_map_difference[k],
                            "cnt": map_rules[r]["cnt"] + 1}
            map_rules_abs[r] = {"d": map_rules_abs[r]["d"] + abs(e_map_difference[k]),
    ...
    def getRuleMapping(self, map_i, k=False):
        ...
        mapping_rule = {"Rule_" + str(i + 1): key for (key, i) in
                        zip(map_i.keys(), range(len(map_i)))}
        return mapping_rule
