...
function InstancesExplClassCompare() {
  ...
  useEffect(() => {
    async function fetchData() {
      const res = await fetch("http://127.0.0.1:5000/instances_class_comparison")
  ...
  function postInstance(instanceId, class1, class2) {
    return async () => {
      await fetch(`http://127.0.0.1:5000/instances_class_comparison/${instanceId}`, {
        method: "POST",
  ...
  return (
    <Container>
      <Row className="mt-3 d-flex align-items-center">
        ...
        <Button variant="dark" disabled={(selectedInstance === null) || (selectedClass === null)  || (selectedSecondClass === null)}
                className="ml-auto p-2"
                onClick={postInstance(selectedInstance, selectedClass, selectedSecondClass)}>Get Explanation</Button>
      </Row>
      <Row>
        <Col lg={8}>
        ...
        <Col lg={2}>
          ...
          <MyTable columns={[{
            Header: "Type",
            accessor: "type"
          }]} data={classes} onCheck={row => e => setSelectedSecondClass(row.values.type)}
