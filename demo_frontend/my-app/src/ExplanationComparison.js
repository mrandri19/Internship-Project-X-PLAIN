...
function ExplanationComparison() {
  ...
  return (
<Container>
<Row className="mt-3 mb-3 d-flex align-items-center"><Col><h2>Explanation Comparison</h2>
          <p>
            Explanation of instance <code>{explanation.instance_id}</code> of dataset <code> {explanation.explainer_info.dataset_name} </code> for the <code> {explanation.explainer_info.classifier_name} </code> and <code> {explanation2.explainer_info.classifier_name} </code> classifiers.
            It belong to class <b>{explanation.target_class}</b> with probability{" "}
            <code>{explanation.prob.toFixed(3)}</code> for <code> {explanation.explainer_info.classifier_name} </code> classifier and <code>{explanation2.prob.toFixed(3)}</code> for <code> {explanation2.explainer_info.classifier_name} </code>. True class: <code> {explanation.true_class} </code>
          </p></Col>
