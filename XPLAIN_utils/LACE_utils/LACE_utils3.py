...
#15-7
def genNeighborsInfo(training_dataset, NearestNeighborsAll, instanceIO, iID, NofKNN, unique_filename, classifier, save=True):
    ...
    for i in range (0, lendataset_nearest_neighbors):           
        ...
        if i==0:
            instanceK_i=Orange.data.Instance(training_dataset.domain, instanceIO)
...
def genNeighborsInfoTraining(training_dataset, NearestNeighborsAll, instanceI, iID, NofKNN, unique_filename, classifier):
    nearest_neighbors = NearestNeighborsAll.kneighbors([instanceI], NofKNN, return_distance=False)
