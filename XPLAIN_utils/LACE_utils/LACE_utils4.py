...
def getUnionXY(y_labels, list_single_compare, y_label_mapping, map_difference, impo_rules_completeC, classif_name=False, predDiff=False, attributes_list_a=False, minvalue=False):
...
def getSingles_Rules(attributes_list, probSingleNoInt,  impo_rules, map_difference, classif_name=False, predDiff=False, attributes_list_a=False, minvalue=False):
    index_rule=1
...
def plotTheInfo_v2(instT, out_data, impo_rules, n_inst, dataname, K, errorFlag, minlenname, minvalue, classname, error, error_single, classif, map_difference, impo_rules_complete, pred_str, save=False):
    ...
    list_single_compare, y_labels, y_label_mapping=getSingles_Rules(attributes_list, probSingleNoInt,  impo_rules, map_difference)

    impo_rules_completeC = ''.join(str(e)+"," for e in list(max(impo_rules_complete, key=len)))[:-1]
  
    y_labels, list_single_compare_2, y_label_mapping=getUnionXY(y_labels, list_single_compare, y_label_mapping, map_difference, impo_rules_completeC)
    ...
    plotbarh(y_labels, list_single_compare_2, title, label2, path+'/Exp_'+namefig+'_deltas.png', colour, min_m, max_m, save=save)
    printMapping_v5(instT, impo_rules, list(map_difference.keys()),y_label_mapping, sep=", ")

def plotTheInfo_v4(instT, out_data, n_inst, dataname, K, classname,  classif, map_difference, pred_str, impo_rules, user_rules=[], save=False):
    classname_f=classname
    ...
    y_labels, predDiffComplete, y_label_mapping, index_rule=getXYExplanationRules(y_labels, predDiffComplete, map_difference, rules=list(set(impo_rules+user_str)), uRules=user_rules)
    ...
    plotbarh(y_labels, predDiffComplete, title, label2, path+'/Expl_'+namefig+'.png', colour, min_m, max_m, user_rules, save=save)


def plotTheInfo_axi(instT, out_data, impo_rules, n_inst, dataname, K, errorFlag, minlenname, minvalue, classname, error, error_single, classif, map_difference, impo_rules_complete, pred_str, axi, save=False):
    classname_f=classname
    ...
    plotbarh_axi(y_labels, list_single_compare_2, title, label2, path+'/SRC_'+namefig+'.png', colour, min_m, max_m, axi)
    printMapping_v5(instT, impo_rules, list(map_difference.keys()),y_label_mapping, sep=", ")
