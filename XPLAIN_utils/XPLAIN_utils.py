...
def compareLocalityLOLD(le1, le2, Sn_inst, reductionMethod="pca", training=False):
    ...
    custom_lines = [plt.Line2D([], [], ls="", marker='.', mec='k', mfc=c, mew=.1, ms=20) for c in
                    colors]
    fig2.legend(custom_lines, [lt[1] for lt in labeltest1.items()], loc='center left',
                bbox_to_anchor=(0.9, .5), fontsize='x-small')
    plt.tight_layout()
    plt.show()
...
def compareLocalityL(le1, le2, Sn_inst, reductionMethod="mca", training=False):
    ...
    if reductionMethod == "pca" or reductionMethod == "mca":
        ...
        fig2.get_axes()[0].legend(custom_lines, [lt[1] for lt in labeltest1.items()],
                                  loc='lower left', bbox_to_anchor=(0, -0.05, 1, 0.35), ncol=10,
                                  mode="expand", borderaxespad=0)
    else:
        reductionMethod = "t-SNE"
        fig2.get_axes()[0].legend(custom_lines, [lt[1] for lt in labeltest1.items()],
                                  loc='lower left', bbox_to_anchor=(0, -0.1, 1, 0.35), ncol=10,
                                  mode="expand", borderaxespad=0)
